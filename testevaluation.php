...
// $liste = $_SESSION['competence'];

$sql ="SELECT * FROM competence ORDER BY id_competence";
$results = $dbo->prepare($sql);
$results->execute();
$rows = $results->fetchAll();

$sql2 =  "SELECT q.question  FROM question q";
$resultquestion = $dbo->prepare($sql2);
$resultquestion->execute();
$rowquestion = $resultquestion->fetchAll();

$title = " - Test d'evaluation";
...
<div class="logoS">
    <svg version="1.1" id="Calque_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1000 1000" style="enable-background: new 0 0 1000 1000" xml:space="preserve">
        ...
        <defs>
            <linearGradient id="0" x1="0.15" y1="0.85" x2="0.85" y2="0.20">
...
<main class="test-container">
    <h2><?= $_SESSION['famille_competence'] ?></h2>
    <?php
    var_dump($_SESSION["competencelist"]);
    for ($i = 0; $i < count($_SESSION["competencelist"]); $i++){
        if($_SESSION["competencelist"][$i] == $rows[$i]['id_competence']){
            ?><p class="description"><?= $rows[$i]['description']; ?></p><?php
        }
    }

    ?>
    <a href="skill.php?metier=<?= $_SESSION["metier"]; ?>&famille=<?= $_SESSION['famille_competence'] ?>&famillesuivante=<?=$_SESSION["famillesuivante"]?>"><button class="btn-search">Valider</button></a>
</main>
